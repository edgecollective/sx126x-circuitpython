...
# Enable all GPS sentences temporarily to debug
...
while True:
    ...
    #if gps_updated:
        #print(f"GPS updated - has_fix: {gps.has_fix}, satellites: {gps.satellites}")
    
    # Brief delay to allow GPS sentences to be received
    time.sleep(0.01)
        
    msg, err = sx.recv()
    #msg, err = sx.recv(timeout_en=True, timeout_ms=1000)
    if len(msg) > 0:
        #print("Message received! Attempting to get GPS fix...")
        
        # Try to get GPS fix for up to 30 seconds after receiving message
        fix_start_time = time.monotonic()
        max_fix_time = 30  # seconds
        
        while not gps.has_fix and (time.monotonic() - fix_start_time) < max_fix_time:
            gps.update()
            time.sleep(0.1)  # Small delay between updates
            
        if gps.has_fix:
            #print(f"GPS fix acquired! lat: {gps.latitude:.6f}, lon: {gps.longitude:.6f}")
            tc.text=f"lat: {gps.latitude:.6f}"
            td.text=f"lon: {gps.longitude:.6f}"
            lat=gps.latitude
            lon=gps.longitude
        else:
            #print("Could not get GPS fix within 30 seconds")
            tc.text=f"lat: timeout"
            td.text=f"lon: timeout"
            lat=0.
            lon=0.
        ...
        outstring=msg_str+","+str(rssi)+","+f"{gps.latitude:.6f}"+","+f"{gps.longitude:.6f}"
        print(outstring)
